...
#ifdef __APPLE__
	// Reduced buffer sizes for iOS to prevent memory heap crashes
...
static u32 s_vulkan_failure_count = 0;
...
void GSDeviceVK::ReportVulkanFailure()
{
	s_vulkan_failure_count++;
#ifdef __APPLE__
	// On iOS, if Vulkan fails multiple times, suggest switching to Metal
	if (s_vulkan_failure_count >= 3)
	{
		Console.Error("Vulkan renderer has failed %u times. Consider switching to Metal renderer for better iOS compatibility.", s_vulkan_failure_count);
		Host::AddKeyedOSDMessage("VulkanFailures", "Vulkan renderer unstable. Switch to Metal for better compatibility.", 10.0f);
	}
#endif
}
...
bool GSDeviceVK::CreateAllocator()
{
	...
	for (u32 i = 0; i < memory_properties.memoryHeapCount; i++)
	{
		...
		// Cap heap usage at 256MB per heap on iOS to prevent memory pressure
		constexpr VkDeviceSize IOS_HEAP_LIMIT = 256 * 1024 * 1024;
...
VkPipeline GSDeviceVK::CreateTFXPipeline(const PipelineSelector& p)
{
	...
	{
#ifdef __APPLE__
		// iOS-specific: Use simplified blending to prevent corruption on MoltenVK
		...
		Console.WriteLn("VK: Using simplified blending for iOS stability");
#else
		// clang-format off
